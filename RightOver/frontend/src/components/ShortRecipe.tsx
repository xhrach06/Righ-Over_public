...
// Show short information about a recipe
function ShortRecipe(props: Props) {
  // Function to get the path for NutriScore image
  const getNutriScoreImagePath = (score: number) =>
    `/static/nutriScore${score}.png`;
  // Function to get the path for EcoScore image
  const getEcoScoreImagePath = (score: number) =>
    `/static/ecoScore${score}.png`;
  // Extracting the first 4 ingredients for display
  const mainIngredients = props.recipe.ingredients.slice(0, 4);
        ...
        <Link to={`/recipe/${props.recipe.id}`} className="recipe-link">
          <div className="row custom-row">
            ...
            <div className="col-4 custom-column align-items-start">
              <h1 className="short-recipe-name">{props.recipe.name}</h1>
            ...
            {/* Column for main ingredients */}
            <div className="col-2 custom-column align-items-start">
              <h4>Main ingredients:</h4>
              <div className="ingredients">
                {/* Displaying the first 4 ingredients */}
